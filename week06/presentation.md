...
1. 백엔드가 먼저 되는지 확인합니다.  
   React를 고치기 전에 `GET http://localhost:8080/api/todos`가 성공해야 합니다.
2. 프론트엔드는 처음부터 파일을 많이 나누지 않습니다.  
   먼저 `App.js` 한 파일로 동작시키고, 그 다음 `useTodos`로 분리합니다.
3. 화면이 이상하면 바로 코드부터 고치지 않습니다.  
   Chrome DevTools의 **Network 탭**에서 요청 URL, Status, Response를 먼저 봅니다.
4. 한 기능씩 완성합니다.  
   목록 조회 → 추가 → 삭제/토글 → 로딩/에러 → 훅 분리 순서입니다.

### 완성 기준

- React 화면을 새로고침하면 Todo 목록이 보인다.
- 입력 후 추가 버튼을 누르면 DB에 저장되고 목록이 갱신된다.
- 삭제 또는 완료 토글 버튼 중 하나 이상이 동작한다.
- Network 탭에서 `GET`, `POST`, `DELETE` 또는 `PATCH` 요청을 확인할 수 있다.
